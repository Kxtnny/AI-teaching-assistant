...
// ---------- feedback ----------
// Replace your FEEDBACK_PROMPT and generatePrivateFeedbackForAll with this version.
...
Return STRICTLY in this exact markdown structure (same headings, same order):
...
- Must not use generic phrases like "good effort" alone.
...
Your next session can be much stronger with just a few clear concept explanations.`;
    ...
    try {
      ...
      const validStructure =
        text.includes("**✅ What went well**") &&
        text.includes("**🛠️ Improvements**") &&
        text.includes("**🌟 Encouragement**");

      map[name] = validStructure
        ? text
        : fallbackStructuredFeedback(name, stats, evidence, room.topic);
    } catch {
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === "join_room") {
    ...
    if (room.phase === "waiting" && room.participants.size < MIN_STUDENTS_TO_START) {
      await postFacilitator(room, `👋 Welcome ${username}! Waiting for one more student to join before we start.`, "waiting");
    }
    ...
    if (room.phase === "waiting" && room.participants.size >= MIN_STUDENTS_TO_START) {
      ...
      await postFacilitator(
        room,
        `🧠 Collaborative mode started!
Instructions:
1) Explain clearly to each other.
2) Build on your peer’s ideas.
3) Correct misconceptions with reasons.
🌳 The tree grows when explanations are accurate/helpful and shrinks when they are misleading.
You have 3 minutes—start now.`,
        "instructions"
      );
    }
